...
<body>
  ...
  <div  style="text-align: center; width: inherit;">
    <h4>Fund From Users Report | {{ get_DeptName(auth('admin')->user()->dept_id,auth('admin')->user()->type) }}</h4>
  </div>
    ...
    <table style="border: 1px solid black; width: inherit;border-collapse: collapse;">
      <thead style="font-size: 12px; width: 100%; border-bottom: 1px solid black">
        <tr>
          ...
          <th scope="col" style="border-right: 1px solid black;text-align: center;">Receipt No</th>
          <th scope="col" style="border-right: 1px solid black;text-align: center;">Deposited To</th>
          <th scope="col" style="border-right: 1px solid black;text-align: center;">Deposited By</th>
        ...
        @foreach($record as $key => $value)
         ...
         <tr>
           <td style="border-right: 1px solid black; border-bottom: 1px solid black; padding:5px;text-align: center;">{{ ++$key }}</td>
            <td style="border-right: 1px solid black; border-bottom: 1px solid black; padding:5px;text-align: center;">{{ $value->receipt_no }}</td>
            <td style="border-right: 1px solid black; border-bottom: 1px solid black; padding:5px;">{{ $u_detail2->name }}</td>
            <td style="border-right: 1px solid black; border-bottom: 1px solid black; padding:5px;">{{ $u_detail->name }}</td>
            <td style="border-right: 1px solid black; border-bottom: 1px solid black; padding:5px;text-align: center;">{{ $value->amount }}</td>
            <td style="border-right: 1px solid black; border-bottom: 1px solid black; padding:5px;text-align: center;">{{ date('d/m/Y',strtotime($value->deposited_on)) }}</td>
         </tr>
        @endforeach
        @else
          <tr>
