					...
					@if(count($provinces) > 0)
						@foreach($provinces as $key => $value)
							@php
								...
								$get_record2 = DB::table('budget_list')->where([
								    'reqst_to' => $value->id,
								    'year' => date('Y'),
								    'budget_type' => 'request',
								    'status' => 'delivered',
								])->sum('amount');
								$get_record3 = DB::table('budget_list')->where([
								    'reqst_to' => $value->id,
								    'year' => date('Y'),
								    'budget_type' => 'request',
								])->where('status','!=','reject')->where('status','!=','delivered')->sum('amount');
								$allocated = $get_record;
								$r =  $allocated - $get_record2 - $get_record3  ;
							@endphp
							<tr>
								<td>{{ ++$key }}</td>
								<td>{{ $value->name }}</td>
