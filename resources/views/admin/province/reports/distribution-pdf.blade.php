...
<body>
  ...
  <div  style="text-align: center; width: inherit;">
    <h4>Budget Allocation Report | {{ get_DeptName(auth('admin')->user()->dept_id,auth('admin')->user()->type) }}</h4>
  </div>
    ...
    <table style="border: 1px solid black; width: inherit;border-collapse: collapse;">
      <thead style="font-size: 12px; width: 100%; border-bottom: 1px solid black">
        <tr>
          ...
          <th scope="col" style="border-right: 1px solid black;text-align: center;">Allocate To</th>
          <th scope="col" style="border-right: 1px solid black;text-align: center;">Amount</th>
          <th scope="col" style="border-right: 1px solid black;text-align: center;">Allocated Date</th>
        </tr>
      </thead>
        ...
        @foreach($record as $key => $value)
         <tr>
           <td style="border-right: 1px solid black; border-bottom: 1px solid black; padding:5px;text-align: center;">{{ ++$key }}</td>
            ...
            <td style="border-right: 1px solid black; border-bottom: 1px solid black; padding:5px;text-align: center;">{{ get_DeptName($value->reqst_to , $value->type) }}</td>
