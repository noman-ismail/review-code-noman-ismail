...
$title = ($res->title !="") ? '<h1 class="header-title ">'.$res->title.'</h1>' : "";
$views = ($res->views !="") ? '<div class="icon"> <i class="icon-eye pr-2"></i>'.total_views($page_id , $post_id).'</div>' : "";
...
$cat = ($res->category !="") ? (Array)get_catByname($res->category) : "";
$c_url = ($res->category !="") ? get_catUrl($res->category) : "";
      ...
      <article class="col-lg-8 col-xl-9 article-column" id="article-column">
        ...
        <div class="blog-bottom">
          <ul class="blog-feat" style="display: flex;justify-content: flex-end;">
            @isset ($res->author)
            @php
              $au = DB::table("authors")->where("id" , $res->author)->select("name")->first();
            @endphp
             <li><i class="icon-user"></i><a>{{$au->name}}</a></li>    
            @endisset
           
            <li><i class="icon-price-tags"></i><a href="{{ $c_url }}">{{ $cat[0] }}</a></li>
            @if ($res->date)
              <li><i class="icon-calendar"></i><a>{{ date("d M Y", strtotime($res->date) ) }}</a></li>
            @endif
            
            <li><i class="icon-eye"></i><a href="#">{{ total_views(get_postid('page_id') , $post_id  )}}</a></li>
          </ul>
        </div>
        <div class="blog-content" id="related-post">
          <div class="isLoading" id="sectionOne">
           ...
           @php
                ...
                $content = do_short_code($content, $res->id , "blogs",$titles, $tags);
...
<script>
  $('.section-child').find('img').css('max-width','100%');
