...
{{-- <header class="title-header main-header">
	<div class="container">
		<div class="header-text">
			<h1 class="header-title">Funds History</h1>
			<ul class="breadcrumb">
				<li><a href="{{ route('user-dashboard') }}">Home</a></li>
				<li><span>Funds History</span></li>
			</ul>
		</div>
	</div>
</header> --}}
								...
								<table>
									...
									@if (count($record) > 0)
										@php
											$i = count($record);
											$total = 0;
											$u_detail = get_user_fund_detail(auth('login')->user()->district,auth('login')->user()->id);
											// dd($record);
										@endphp
										@foreach ($record as $key => $val)
											@php
												(!empty($val->receipt_no)) ? $total = $total + $val->amount : $total = $total - $val->amount ;
												$user_detail = get_user_detail($val->user_id);
											@endphp
											@if (!empty($val->receipt_no))
												<tr>
													...
													<th>
														<a href="{{ route('funds-history')."?print_id=".$val->id }}">
											...
											@else
												<tr>
													<td>{{ ++$key }}</td>
													<td colspan="2">Transfered To {{ get_DeptName(auth('login')->user()->province,'province') }}</td>
													<td>{{ "-".$val->amount }}</td>
													<td>{{ $total }}</td>
													<td>{{ (!empty($val->date))?date('d/m/Y' , strtotime($val->date)):"" }}</td>
													<th></th>
												</tr>
											@endif
										@endforeach
										<tr>
											...
											<td>{{ $u_detail['total_collection'] }}</td>
											<th class="text-right">Total Balance:</th>
											<td>{{ $u_detail['balance'] }}</td>
										</tr>
									@else
										<tr>
											<td class="text-center" colspan="8">There is no record. &#128542;</td>
										</tr>
									@endif
								</table>
							</div>
